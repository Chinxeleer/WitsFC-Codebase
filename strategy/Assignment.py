...
def hungarian_algorithm(cost_matrix):
    cost_matrix = cost_matrix.copy()  # Copy to avoid modifying the original
    ...
    starred_zeros = np.zeros((num_rows, num_cols), dtype=bool)
    ...
    def find_star_in_row(row):
        star_col = np.where(starred_zeros[row])[0]
    ...
    while np.sum(col_covered) < num_cols:
        ...
        if star_col == -1:
            ...
            while True:
                ...
                col = find_prime_in_row(star_row)
                path.append((star_row, col))
